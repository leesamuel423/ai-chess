...
def tapered_eval(board, openingEval, endgameEval):
    """
    Tapered Board Evaluation
    Notes: Optimize such that phase calculation doesn't happen unless there is a capture, otherwise phase is constant
    ie: if capture -> phase calculation -----> tapered_eval (openingEval, endgameEval, phase)

    @param board: current board state
    @param openingEval: evaluation of piece in respective material opening/midgame heuristic
    @param endgameEval: evaluation of piece in respective material endgame heuristic
    @return: evaluation
    """
    phase = total_phase

    # white pieces
    phase -= len(board.pieces(chess.PAWN, chess.WHITE)) * pawn_phase
    ...
    # black pieces
    phase -= len(board.pieces(chess.PAWN, chess.BLACK)) * pawn_phase
    ...
    eval = ((openingEval * (256 - phase)) + (endgameEval * phase)) / 256
